...
def parse_arguments():
    ...
    parser.add_argument('--input_dir', type=str,default=r"in_dir/", help="Input folder with images to turn seamless")

    parser.add_argument('--out_dir', type=str,default=r"out_seamless/", help="Directory where output will be saved")
...
if __name__ == "__main__":
    ...
    out_tiled = args.out_dir +r"/tiled/" # were tiled image will be saved
    out_image = args.out_dir +r"/image/" # were the modify texture will be saved
    if not os.path.exists(args.out_dir): os.mkdir(args.out_dir)
    if not os.path.exists(out_tiled): os.mkdir(out_tiled)
    if not os.path.exists(out_image): os.mkdir(out_image)

    for fl in os.listdir(args.input_dir):

        # load PIL image
        print(args.input_dir+"/"+fl)
        im=cv2.imread(args.input_dir+"/"+fl)
